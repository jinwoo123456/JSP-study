...
//	서블릿에서 멤버변수 선언
	...
	/*
	클라이언트가 최초로 요청했을 때 서블릿 인스턴스가 생성되는데 이 때 최초 1번만
	호출되는 수명주기 메서드인 init()에서 DB연결 처리를 한다.
	 */
	@Override
	public void init() throws ServletException {
		/*
		서블릿 내에서 application 내장객체를 얻어온다. 모델2 방식에서는
		서블릿 클래스가 먼저 요청을 받기 때문에 모델1 방식과 같이 JSP에서 매개변수로
		내장 객체를 전달할 수 없다. 따라서 각 내장객체를 얻어올 수 있는 메서드가 존재한다.
		*/
