...
<body>
    <h3>변수 선언 및 할당</h3>
    <!-- 
    EL 및 JSTL은 영역에 저장된 속성을 이용해서 연산하거나 출력한다.
    따라서 스크립트렛에서 선언한 변수는 EL에서 사용할 수 없다.
    따라서 num1은 아무값도 출력되지 않는다.
     -->
    스크립틀릿에서 선언한 변수 : ${ num1 }  <br />    
    page 영역에 저장된 변수 : ${ num2 } <br />
    
    <!-- 
    Tomcat8.0 부터는 EL에서 변수할당을 할 수 있다.하지만 할당과 동시에 출력되므로 할당문 뒤에
    ;''을 붙여줘야 출력되지 않는다. 즉, EL에서 변수할당은 거의 사용되지 않는다.
     -->
    ...
    num1 * num2 : ${ num1 * num2 } <br />
    <!-- 
    나눗셈에서는 /와 %대신에 div, mod를 사용할 수 있다. 또한 자동형변환이 적용되므로
    정수끼리의 연산에서 실수의 결과를 반활할 수 있다.
     -->
    ...
    num3 mod num4 : ${ num3 mod num4 }
   	...
   	<!-- 
   	EL에서는 + 연산자로 덧셈 기능만 수행할 수 있다. 문자열의 연겨렝는 사용되지 않는다.
   	 -->
    <h3>+ 연산자는 덧셈만 가능</h3> 
    <!-- 문자열 34가 자동으로 정수로 변환되어 연산된다.
    Integer.parseInt()를 정용한것과 동일한 결과가 반환된다. --> 
    num1 + "34" : ${ num1 + "34" } <br />
    <!-- 하지맛 숫자와 문자열 혹은 문자열과 문자열의 경우에는 덧셈 연산이
    불가능하므로 예외가 발생된다. -->
    num2 + "이십" : \${num2 + "이십" }  <br /> 
    "삼십" + "사십" : \${"삼십" + "사십" } 
